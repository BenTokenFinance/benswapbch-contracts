...
contract SendingOfferingTokensToRaisingTokenConcluderIboV2Concluder is IIboV2Concluder {
    using SafeMath for uint256;
    ...
    constructor() public {
        _name = "SendingOfferingTokensToRaisingTokenConcluder";
        _template = msg.sender;
        _status = 0;
        _isInitialized = false;
    }

    function initialize0(address __ibo, address __raisingToken, address __offeringToken, address __owner) external {
    ...
    function conclude() external override {
        ...
        _offeringToken.safeTransfer(address(IIboV2(_ibo).raisingTokenConcluder()), _offeringToken.balanceOf(address(this)));
...
contract SendingOfferingTokensToRaisingTokenConcluderIboV2ConcluderTemplate is IIboV2ConcluderTemplate {
    ...
    function createConcluder(address ibo, address raisingToken, address offeringToken, address owner, bytes memory callData) external override returns (address concluder) {
        bytes memory bytecode = type(SendingOfferingTokensToRaisingTokenConcluderIboV2Concluder).creationCode;
        ...
        SendingOfferingTokensToRaisingTokenConcluderIboV2Concluder(concluder).initialize0(ibo, raisingToken, offeringToken, owner);
        SendingOfferingTokensToRaisingTokenConcluderIboV2Concluder(concluder).initialize();
