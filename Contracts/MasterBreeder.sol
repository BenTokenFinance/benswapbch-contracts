...
// MasterBreeder is the master of GreenBen. He can make GreenBen and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GreenBen is sufficiently
...
contract MasterBreeder is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EBENs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accGreenBenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGreenBenPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // GreenBen tokens created per block.
    uint256 public emissionPerBlock;
    // Bonus muliplier for early GreenBen makers.
    ...
    // The block number when GreenBen mining starts.
    ...
    // View function to see pending EBENs on frontend.
    function pendingGreenBen(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accGreenBenPerShare = pool.accGreenBenPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ebenReward = multiplier.mul(emissionPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGreenBenPerShare = accGreenBenPerShare.add(ebenReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accGreenBenPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ebenReward = multiplier.mul(emissionPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        eben.mint(devaddr, ebenReward.div(10));
        eben.mint(address(this), ebenReward);
        pool.accGreenBenPerShare = pool.accGreenBenPerShare.add(ebenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterBreeder for EBEN allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accGreenBenPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeGreenBenTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accGreenBenPerShare).div(1e12);
    ...
    // Safe GreenBen transfer function, just in case if rounding error causes pool to not have enough EBENs.
    function safeGreenBenTransfer(address _to, uint256 _amount) internal {
        uint256 bal = eben.balanceOf(address(this));
        if (_amount > bal) {
            eben.transfer(_to, bal);
        } else {
            eben.transfer(_to, _amount);
        }
    }
