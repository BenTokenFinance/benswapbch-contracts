...
interface IIboV2 {
    function raisingTokenConcluder() external view returns (IIboV2Concluder);
}

contract TimeLockOfferingTokensIboV2Concluder is IIboV2Concluder {
    using SafeMath for uint256;
    ...
    function conclude() external override {
        ...
        // Let raising token concluder finish first to avoid excessive gas fee
        if (IIboV2(_ibo).raisingTokenConcluder().status() < 999) return;

        TransferHelper.safeApprove(address(benLockFactory.feeToken()), address(benLockFactory), uint(-1));
        TransferHelper.safeApprove(address(_offeringToken), address(benLockFactory), uint(-1));

        uint256 amount = _offeringToken.balanceOf(address(this));

        if (amount > 0) {
            for (uint256 i = 0; i < unlockDeltas.length; ++i) {
                address benLock = benLockFactory.createBenLock(address(_offeringToken), _owner, block.timestamp + unlockDeltas[i], amount.mul(lockedRateBPs[i]).div(10000));
                benLocks.push(benLock);
            }

            _offeringToken.safeTransfer(sendTo, _offeringToken.balanceOf(address(this)));
        }
    ...
    function feeNeeded() external view returns (uint256) {
        uint256 feeBalance = benLockFactory.feeToken().balanceOf(address(this));
        uint256 fee = benLockFactory.creationFee().mul(unlockDeltas.length);
        if (feeBalance >= fee) return 0;
        return fee.sub(feeBalance);
    }
}
...
contract TimeLockOfferingTokensIboV2ConcluderTemplate is IIboV2ConcluderTemplate {
    ...
    function createConcluder(
        ...
        bytes memory bytecode = type(TimeLockOfferingTokensIboV2Concluder).creationCode;
        ...
        TimeLockOfferingTokensIboV2Concluder(concluder).initialize0(ibo, raisingToken, offeringToken, owner, benLockFactory);
