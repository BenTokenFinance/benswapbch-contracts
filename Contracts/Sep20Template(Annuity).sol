...
// File: @openzeppelin/contracts/token/SEP20/ISEP20.sol


// OpenZeppelin Contracts (last updated v4.6.0) (token/SEP20/ISEP20.sol)
...
// File: @openzeppelin/contracts/token/SEP20/extensions/ISEP20Metadata.sol


// OpenZeppelin Contracts v4.4.1 (token/SEP20/extensions/ISEP20Metadata.sol)

pragma solidity ^0.8.0;


/**
 * @dev Interface for the optional metadata functions from the SEP20 standard.
 *
 * _Available since v4.1._
 */
interface ISEP20Metadata is ISEP20 {
    /**
...
// File: @openzeppelin/contracts/token/SEP20/SEP20.sol


// OpenZeppelin Contracts (last updated v4.6.0) (token/SEP20/SEP20.sol)
 ...
 * conventional and does not conflict with the expectations of SEP20
 * applications.
 *
 ...
 */
contract SEP20 is Context, ISEP20, ISEP20Metadata {
     ...
     * Ether and Wei. This is the value {SEP20} uses, unless this function is
     * overridden;
     *
     * NOTE: This information is only used for _display_ purposes: it in
     * no way affects any of the arithmetic of the contract, including
     * {ISEP20-balanceOf} and {ISEP20-transfer}.
     */
     ...
     * problems described in {ISEP20-approve}.
     *
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "SEP20: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(fromBalance >= amount, "SEP20: transfer amount exceeds balance");
        unchecked {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(accountBalance >= amount, "SEP20: burn amount exceeds balance");
        unchecked {
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "SEP20: insufficient allowance");
            unchecked {
...
/// @title An Annuity like Smart Contract
/// @notice You can use this contract to stake and earn
/// @dev Inherits SEP20 functionality
contract AnnuityToken is SEP20 {
    /*** Type Declarations ***/
    ...
    uint256 private _endInterestTime;
    ...
    // APY basis points
    uint16 private _apyBP = 10000;

    // Payout period in seconds
    uint256 private _payoutPeriod = SECONDS_PER_YEAR;

    // One represented as a 60.18-decimal fixed-point number
    uint256 private constant ONE_FIXED_POINT = 1000000000000000000;
    ...
    function initialize(address creator, string memory name_, string memory symbol_, uint256 initialSupplyNorm, uint256 payoutPeriod, uint256 endInterestTime, uint16 apyBP) public {
        require(!_isInitialized, 'Sep20: not initialized!');
        require(initialSupplyNorm <= 1e27, 'too much initial supply!');
        require(payoutPeriod > 0, 'payoutPeriod cannot be 0!');
        require(apyBP > 0, 'APY BP cannot be 0!');
        require(apyBP <= 30000, 'APY BP is too high!');
        require(endInterestTime > block.timestamp, 'Invalid endInterestTime!');
        require(endInterestTime - block.timestamp <= SECONDS_PER_YEAR * 51, 'Invalid endInterestTime!');

        _name = name_;
        _symbol = symbol_;

        // Create initial supply
        _mint(creator, initialSupplyNorm);

        //Approves the initial supply for the sender to use
        bool success = approve(creator, initialSupplyNorm);
        require(success, "'approve' failure");

        _apyBP = apyBP;
        _payoutPeriod = payoutPeriod;

        // solhint-disable-next-line not-rely-on-time
        _endInterestTime = endInterestTime;
    ...
    function getApyBP() public view returns (uint16) {
        return _apyBP;
    }

    function getPayoutPeriod() public view returns (uint256) {
    ...
    /// @return The tokens you would collect if you executed a collect payout at the provided timestamp
    ...
    /// @param addr The address that is asssociated with the Account
    /// @param timeStamp The future timestamp to determine how much tokens you would have at that point in time
    /// @return The tokens you would have if you kept staking up until the provided timestamp
    ...
    function getCurrentEstimate(address addr) external view returns (uint256) {
        if (_isAccountInPayout()) {
            return estimateCollectPayout(addr, block.timestamp);
        }

        return estimateStakeBalance(addr, block.timestamp);
    }
    ...
    /// @return The tokens that would be destroyed during the force payout action
    ...
    */
    ...
    function _getAnnualCompoundedRate() private view returns(uint256) {
        return (10000 + _apyBP) * ONE_FIXED_POINT / 10000;
    }
    ...
    */
    function _calculateInterest(uint256 stakedAmountNorm, uint256 secondsStakedNorm) private view returns (uint256) {
        // Convert seconds staked into fixed point number
        ...
        // Continuously compound the interest
        uint256 compoundedRate = PRBMathUD60x18.pow(_getAnnualCompoundedRate(), percentageYearStaked);
    ...
    */
    function _calculatePayoutToAccount(address addr, uint256 timeStamp) private view returns (uint256) {
        ...
        // Set payout period in seconds
        uint256 payoutPeriodInSecondsInt = _payoutPeriod;
...
contract AnnuitySep20Template is ISep20Template {
    uint256 private _count = 0;
    string private _name = "Annuity";
    ...
    function createToken(bytes memory callData) external override returns (address token) {
        bytes memory bytecode = type(AnnuityToken).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(callData, msg.sender, _count));
        assembly {
            token := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        (bool success, ) = token.call(abi.encodePacked(bytes4(keccak256(bytes("initialize(address,string,string,uint256,uint256,uint256,uint16)"))), callData));
