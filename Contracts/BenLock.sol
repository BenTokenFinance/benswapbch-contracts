...
contract BenLock {
    ...
    function initialize(address _token, uint256 _unlockTime, address _owner) external {
...
contract BenLockFactory {
    ...
    constructor(address _admin, ISingleAddressValidator _priviledgeValidator, IBEP20 _feeToken, address _feeTo, uint256 _creationFee) public {
        adminAddress = _admin;
    ...
    function createBenLock(address token, address creator, uint256 unlockTime, uint256 amount) external returns (address benLock) {
        ...
        bytes memory bytecode = type(BenLock).creationCode;
        ...
        BenLock(benLock).initialize(token, unlockTime, creator);
