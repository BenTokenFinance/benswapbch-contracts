...
/**
 * @dev Interface of the SEP20 standard as defined in the EIP.
 */
interface ISEP20 {
    /**
...
contract LiquidityGeneratorToken is ISEP20, Ownable {
    ...
    function initialize0(address rt, address la) public onlyOwner {
    ...
    function initialize(
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_rt);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "SEP20: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "SEP20: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function excludeFromReward(address account) public onlyOwner {
        // require(account != _rt, 'We can not exclude Uniswap router.');
    ...
    function _transfer(
        ...
        require(from != address(0), "SEP20: transfer from the zero address");
        require(to != address(0), "SEP20: transfer to the zero address");
...
contract LiquidityGeneratorSep20Template is ISep20Template {
    uint256 private _count = 0;
    string private _name = "LiquidityGenerator";
    ...
    constructor(address rt, address la) {
        _rt = rt;
        _la = la;
    }

    function createToken(bytes memory callData) external override returns (address token) {
        bytes memory bytecode = type(LiquidityGeneratorToken).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(callData, msg.sender, _count));
        assembly {
            token := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        LiquidityGeneratorToken(payable(token)).initialize0(_rt, _la);

        (bool success, ) = token.call(abi.encodePacked(bytes4(keccak256(bytes("initialize(address,string,string,uint256,address,uint16,uint16,uint16)"))), callData));
