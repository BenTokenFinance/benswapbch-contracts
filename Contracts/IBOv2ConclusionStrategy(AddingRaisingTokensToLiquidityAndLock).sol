...
interface IBenLockFactory {
    function feeToken() external view returns (IBEP20);
...
contract AddRaisingTokensToLiquidityAndLockIboV2Concluder is IIboV2Concluder {
    using SafeMath for uint256;
    ...
    uint256 public lockedOfferingToken;
    uint256 public lockedRaisingToken;
    address[] public benLocks;
    ...
    constructor() public {
        _name = "AddingRaisingTokensToLiquidityAndLock";
        _template = msg.sender;
        _status = 0;
        _isInitialized = false;
    }

    function initialize0(address __ibo, address __raisingToken, address __offeringToken, address __owner, address __benLockFactory, address __factory, address __router) external {
        ...
        factory = IUniswapV2Factory(__factory);
        router = IUniswapV2Router02(__router);
    ...
    function liquidityPair() public view returns(address) {
        return factory.getPair(address(_raisingToken), address(_offeringToken));
    }
    ...
    function _token2Token(
        ...
        TransferHelper.safeApprove(_FromTokenContractAddress, address(router), uint(-1));
    ...
    function _uniDeposit(
        ...
        TransferHelper.safeApprove(_ToUnipoolToken0, address(router), uint(-1));
        TransferHelper.safeApprove(_ToUnipoolToken1, address(router), uint(-1));
    ...
    function addLiquidity() private {
        address t0 = address(_raisingToken);
        address t1 = address(_offeringToken);
        uint256 b0 = _raisingToken.balanceOf(address(this));
        uint256 b1 = _offeringToken.balanceOf(address(this));
        require(b0 > 0 && b1 > 0, "can't add liquidity with 0 balance.");

        lockedOfferingToken = b1;
        lockedRaisingToken = b0;
        
        address pair = liquidityPair();

        if (pair == address(0)) {
            // create the pair if it doesn't exist yet
            pair = IUniswapV2Factory(factory).createPair(t0, t1);
        } else {
            (uint256 res0, uint256 res1, ) = IUniswapV2Pair(pair).getReserves();
            if (res0 > 0 && res1 > 0) {
                if (IUniswapV2Pair(pair).token0() == t1) {
                    t0 = t1;
                    t1 = address(_raisingToken);
                    uint256 btemp = b0;
                    b0 = b1;
                    b1 = btemp;
                }

                // zap in tokens to the pair
                (uint256 tb0, uint256 tb1) = _swap(pair, t0, t0, t1, b0);
                _uniDeposit(t0, t1, tb0, tb1);
                _swap(pair, t1, t0, t1, b1);
            }
        }

        _raisingToken.safeTransfer(pair, _raisingToken.balanceOf(address(this)));
        _offeringToken.safeTransfer(pair, _offeringToken.balanceOf(address(this)));
    ...
    function conclude() external override {
        ...
        if (_status == 0) {
            ...
            address pair = liquidityPair();
            TransferHelper.safeApprove(pair, address(benLockFactory), uint(-1));

            uint256 amount = IUniswapV2Pair(pair).balanceOf(address(this));

            if (amount > 0) {
                for (uint256 i = 0; i < unlockDeltas.length; ++i) {
                    address benLock = benLockFactory.createBenLockWithPriviledge(pair, _owner, block.timestamp + unlockDeltas[i], amount.mul(lockedRateBPs[i]).div(10000));
                    benLocks.push(benLock);
                }

                TransferHelper.safeTransfer(pair, sendTo, IUniswapV2Pair(pair).balanceOf(address(this)));
            }
    ...
    function lockerCount() external view returns (uint256) {
        return unlockDeltas.length;
    }

    function raisingTokenBalance() external view returns (uint256) {
        return _raisingToken.balanceOf(address(this));
    }

    function offeringTokenBalance() external view returns (uint256) {
        return _offeringToken.balanceOf(address(this));
    }

    function canAddLiquidity() external view returns(bool) {
        return _raisingToken.balanceOf(address(this)) > 0 && _offeringToken.balanceOf(address(this)) > 0;
    }
}
...
contract AddRaisingTokensToLiquidityAndLockIboV2ConcluderTemplate is IIboV2ConcluderTemplate {
    uint256 private _count = 0;
    address public benLockFactory;
    address public factory;
    address public router;

    constructor(address _benLockFactory, address _factory, address _router) public {
    ...
    function createConcluder(
        ...
        bytes memory bytecode = type(AddRaisingTokensToLiquidityAndLockIboV2Concluder).creationCode;
        ...
        AddRaisingTokensToLiquidityAndLockIboV2Concluder(concluder).initialize0(ibo, raisingToken, offeringToken, owner, benLockFactory, factory, router);
