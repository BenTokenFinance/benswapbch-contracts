...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("BenSwap Lottery Ticket", "BSLT") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function reset() external onlyAdmin {
        ...
        if (burnAmount>0) {
            eben.safeTransfer(address(eben), burnAmount);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        eben.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        eben.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            eben.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            eben.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyOwner {
        eben.safeTransfer(address(msg.sender), _amount);
